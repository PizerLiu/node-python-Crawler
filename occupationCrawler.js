		...
		if(htmlUrl.indexOf('https://www.nowcoder.com') != -1){
			https.get(htmlUrl, function (res) {	
			        ...
			        res.on('end', async function () {
						...
						// var xzx = excludeSpecial($('.rec-job>dl').text()).replace("undefined","")
						//解析公司信息
						...
						let rsaa = excludeSpecial($('.rec-job>dl').text());
	
						objectInfo.companyPostRequirementText = rsaa.replace(/(&mdash;)|(&rdquo;)|(&ldquo;)|(&middot;)|(nbsp;)|(&nbsp;)|(undefined)|(&lt;)|(br)|(&gt;)/g,"").trim()					
		...
		if(htmlUrl.indexOf('http://www.yingjiesheng.com') != -1){
		    ...
		    .end(function (err, html) {
	            ...
	            if(sres != undefined){
				    ...
				    var $ = cheerio.load(sres, {decodeEntities: false});
				    ...
				    $('div>.jobIntro').each(function(i,ele){
				    	if(i == 0){
				    		let rsaa = $(this).text().replace(/[email protected]/g,"")
			    			objectInfo.companyPostRequirementText = rsaa.replace(/(&mdash;)|(&rdquo;)|(&ldquo;)|(&middot;)|(nbsp;)|(&nbsp;)|(undefined)|(&lt;)|(br)|(&gt;)/g,"").trim();
			    			objectInfo.companyPostRequirementHtml = $(this).html()	
				    	}
				    	if(i == 1){
				    		objectInfo.companyInfo = $(this).text().replace(/(&mdash;)|(&rdquo;)|(&ldquo;)|(&middot;)|(nbsp;)|(&nbsp;)|(undefined)|(&lt;)|(br)|(&gt;)/g,"").trim()
				    	}
			        	resolve(objectInfo);
				    })
				    
		    	}else{
		    		console.log("undefined>>>>>",htmlUrl)
		    		resolve(objectInfo);
		    	}
...
(async() => {
	...
	//6小时一刷新
	var timeTask = setInterval(async function() {
		try {
	        await init();
